...
{
    int presence_of_bit = 1 << b & input;   // take 1 and shift left b times, then use bitwise & to check if there is 1 there.
    if(presence_of_bit == 0){
        return false;
    }
    else{
        return true;
...
{
    int presence_of_bit = 1 << 31 & input;   // take 1 and shift left 31 times, then use bitwise & to check if there is 1 there.
    if(presence_of_bit == 0){
        return false;
    }
    else{
...
{
    int count = 0;                         // counts number of 1's present in bit number.
    while(input != 0){                     // while input is not exhausted, go through and add to count each time a bit is = 1.
        count += input & 1;
        input = input >> 1;                // shifts bit right so the while loop doesn't get into an infinite loop.
    }
    return count;
}
...
{
    int byte_value = input >> (b * 8) & 0xFF;          // move input b * 8 times to the right. Then compare against 0xFF.
    return byte_value;
}
...
{
    int byte_to_set = 0xFF << (b * 8);
    byte_to_set = ~byte_to_set;
    byte_to_set = byte_to_set & input;
    
    int value_to_set = value << (b * 8);
    int output = value_to_set | byte_to_set;
    return output;
}
...
int Negate( int input )
{
  // Note, it may help to do the challenge question (see below) before implementing this one...
    ...
    int num_to_negate = Increment(~input);
    return num_to_negate;
}
...
int Increment( uint32_t x ){
    ...
    if(int_to_increment == 32) {
        return 0;
    }
    int bit_mask = 0x1 << int_to_increment;
    x = bit_mask | x;
    int new_bit_mask = 0xFFFFFFFF << int_to_increment;
    return new_bit_mask & x;
}
